...
@section('content')
    <form id="post-form" class="form-group col-md-12" method="post" action="{{ route('info.update', $info->id) }}" enctype="multipart/form-data" role="form">
        @method('PUT')
        @csrf
        <div class="form-group row">
            <label for="company_name" class="col-sm-3 col-form-label">Tên công ty</label>
            <div class="col-sm-8">
                <input name="company_name" type="text" class="form-control" id="company_name" placeholder="Tên công ty" value="{{ $info->company_name }}" required/>
            </div>
        </div>
        <div class="form-group row">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8">
                <input name="phone" type="text" class="form-control" id="phone" placeholder="số điện thoại" value="{{ $info->phone }}" required/>
            </div>
        </div>
        <div class="form-group row">
            <label for="website" class="col-sm-3 col-form-label">Website</label>
            <div class="col-sm-8">
                <input name="website" type="text" class="form-control" id="website" placeholder="Địa chỉ website công ty" value="{{ $info->website }}" required/>
            </div>
        </div>
        <div class="form-group row">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8">
                <textarea name="description" type="text" class="form-control" id="description"
                          placeholder="Giới thiệu về công ty" rows="6" required> {{ $info->description }}
                </textarea>
            </div>
        </div>
        <div class="form-group row">
